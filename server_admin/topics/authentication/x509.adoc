...
* Optionally validates whether the policies in the certificate matches the expected certificate policies
...
`Validate Certificate Policy` (optional)::
Verifies one or more policy OIDs as defined in the Certificate Policy extension. See link:https://tools.ietf.org/html/rfc5280#section-4.2.1.4[RFC5280, Section-4.2.1.4]. Leave the parameter empty to disable the Certificate Policy validation. Multiple policies should be separated using a comma.

`Certificate Policy Validation Mode` (optional)::
